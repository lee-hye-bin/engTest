...
function createCalendar(date){
  ...
  for(let day = 1; day <= lastDay; day++){
    ...
    if(
      ...
      day < today.getDate() 
    ){cell.classList.add('impossible_day');}
    ...
    if (dateCell.getDay() === 6 || dateCell.getDay() === 0) {
      cell.style.cursor = 'default'; // 클릭 비활성화
    }
...
calendarBody.addEventListener('click', (e) => {
  if (
    ...
    const clickedDay = e.target.textContent;
    const clickedDate = new Date(today.getFullYear(), today.getMonth(), clickedDay);
    const { monday, friday } = getWeekStartAndEnd(clickedDate);
